package com.smf.tutorial.actionHandlers;
 ...
 *
 */
public class SelectorEvalActionHandler extends BaseProcessActionHandler {
  ...
  public void execute() {
    ...
    if (StringUtils.equals("DOWNLOAD", mode)) {
      try {
        doDownload(request);
      } catch (Exception e) {
      ...
      return;
    } else if (StringUtils.equals("BROWSE", mode)) {
      try {
        doBrowse(request);
      } catch (IOException e) {
  ...
  private void doBrowse(HttpServletRequest request) throws IOException {
    ...
    ReportingUtils.ExportType expType = ReportingUtils.ExportType.HTML;
  ...
  private void doDownload(HttpServletRequest request) throws IOException {
    ...
    final String strFileName = (String) parameters.get("fileName");
    final String tmpFileName = (String) parameters.get("tmpfileName");
    ReportingUtils.ExportType expType = null;

    if (strFileName.endsWith("." + ReportingUtils.ExportType.PDF.getExtension())) {
      expType = ReportingUtils.ExportType.PDF;
    } else if (strFileName.endsWith("." + ReportingUtils.ExportType.XLS.getExtension())) {
      expType = ReportingUtils.ExportType.XLS;
    } else if (strFileName.endsWith("." + ReportingUtils.ExportType.XLSX.getExtension())) {
      expType = ReportingUtils.ExportType.XLSX;
    } else if (strFileName.endsWith("." + ReportingUtils.ExportType.CSV.getExtension())) {
      expType = ReportingUtils.ExportType.CSV;
    } else {
  ...
  private void handleReportResponse(HttpServletRequest request, ReportingUtils.ExportType expType,
      ...
      if (includeReportAsAttachment) {
        ...
        if (userAgent != null && userAgent.contains("MSIE")) {
          ...
          response.setHeader("Content-Disposition",
              "attachment; filename=\"" + MimeUtility.encodeWord(tmpFileName, "UTF-8", "Q") + "\"");
        }
      }
      ...
      try {
        ...
        log4j.error("Error al volcar el archivo '" + tmpFileName + "': " + e.getMessage(), e);
        throw e;
      } finally {
        if (file.exists()) {
          file.delete();
  ...
  protected String getReportTemplatePath(ReportingUtils.ExportType expType, ReportDefinition report,
  ...
  protected void doGenerateReport(JSONObject result, Map<String, Object> parameters,
    ...
    final ReportingUtils.ExportType expType = getExportType(action);
    ...
    jrParams.put("evaluation_id", params.get("evaluation_id"));
    ...
    if (expType.equals(ReportingUtils.ExportType.HTML)) {
  ...
  private ReportingUtils.ExportType getExportType(String action) {
    if (ReportingUtils.ExportType.XLS.hasExtension(action)) {
      return ReportingUtils.getExcelExportType();
    }
    return ReportingUtils.ExportType.getExportType(action);
  }
  ...
  private void doJRExport(String jrTemplatePath, ReportingUtils.ExportType expType, String strFileName,
      Map<String, Object> parameters) {
    ...
    try {
      if (ReportingUtils.ExportType.HTML.equals(expType)) {
